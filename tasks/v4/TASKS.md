...
| v4-022 | WF9 LI Sourcing — search/post_engagers/profile_visitors/external_post → Leads_Raw + dedup Registry étendu | 🤖 | P0 | 🚧 Partiel — 2026-07-04 (WF9 staging actif dry-run + normalisation/dedup/cap 100 smokés ; Unipile live + append Sheets restent à faire) | v4-020 | 6h |
...
| v4-024b | WF12 NEW — Health monitor LI : polling Unipile + parsing inbox compte → détecte captcha/warning email/acceptance<20%/bouton Suivre → auto-pause + alerte UI + email Thomas. Tab `LI_Health` Index | 🤖 | P0 | 🚧 Partiel — 2026-07-04 (WF12 staging actif dry-run + `/api/li-health` POST/historique + smoke `paused_captcha` OK ; polling Unipile/live alertes restent à faire) | v4-024 | 4h |
| v4-024c | Bandeau dashboard `<LIHealthBanner>` rouge si status != active + raison + ETA reprise | 🤖 | P0 | 🚧 Partiel — 2026-07-04 (bandeau livré ; WF12 dry-run produit `paused_captcha`, preuve UI live après persistance Sheets restante) | v4-024b | 2h |
| v4-025 | WF10 LI Outreach — invitation perso IA + cold DM post-acceptance + pacing + appel `checkHealth()` avant chaque action | 🤖 | P0 | 🚧 Partiel — 2026-07-04 (WF10 staging actif dry-run : pause `paused_*`, cap weekly 100, acceptance DM planifiés ; envois Unipile/Sheets live restent à faire) | v4-024, v4-024b, v4-022 | 5h |
| v4-025b | WF10 mix d'actions organiques — 1 like + 1 profile view toutes les N invits (pattern humain) | 🤖 | P0 | 🚧 Partiel — 2026-07-04 (action organique dry-run toutes les 3 invitations + log payload ; like/view Unipile live restent à faire) | v4-025 | 2h |
| v4-026 | WF11 Setter LI — webhook `message.received` → réutilise pipeline Setter (channel=linkedin) | 🤖 | P0 | 🚧 Partiel — 2026-07-04 (WF11 staging actif dry-run : `message.received` → draft `channel=linkedin`, Calendly texte ; pipeline Setter/Sheets/DM live restent à brancher) | v4-005, v4-020 | 4h |
| v4-027 | Inbox unifié email + LI dans `<ConversationThread>` (channel badge) | 🤖 | P0 | ✅ | v4-013, v4-026 | 2h |
| v4-028 | Tests + E2E Phase 2 : sourcing → invitation → DM → reply → Setter LI → Calendly + smoke captcha simulé → pause auto | 🤝 | P0 | 🚧 Partiel — 2026-07-04 (script HMAC + checklist WF9-WF12 + smokes n8n dry-run OK ; smoke Unipile réel à exécuter) | v4-022→027 | 4h |
...
| v4-032 | Pacing fin avancé : tuning ramp-up (déjà livré v4-024) + alertes santé granulaires + dashboards LI_Health historiques | 🤖 | P1 | 🚧 Partiel — 2026-07-04 (`LI_Health_History` append via `/api/li-health` POST prêt ; données WF12 live en attente) | v4-024, v4-024b | 2h |
