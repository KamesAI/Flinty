...
| v4-025 | WF10 LI Outreach — invitation perso IA + cold DM post-acceptance + pacing + appel `checkHealth()` avant chaque action | 🤖 | P0 | 🚧 Partiel — 2026-07-04 (WF10 staging persist-ready : pause `paused_*`, cap weekly 100, acceptance DM + route `statut_li` testées ; envois Unipile/Sheets live restent à faire) | v4-024, v4-024b, v4-022 | 5h |
...
| v4-026 | WF11 Setter LI — webhook `message.received` → réutilise pipeline Setter (channel=linkedin) | 🤖 | P0 | 🚧 Partiel — 2026-07-04 (WF11 staging persist-ready : `message.received` → draft `channel=linkedin`, route turns LI + HMAC API testées ; classify/generate/DM live restent à brancher) | v4-005, v4-020 | 4h |
| v4-027 | Inbox unifié email + LI dans `<ConversationThread>` (channel badge) | 🤖 | P0 | ✅ | v4-013, v4-026 | 2h |
| v4-028 | Tests + E2E Phase 2 : sourcing → invitation → DM → reply → Setter LI → Calendly + smoke captcha simulé → pause auto | 🤝 | P0 | 🚧 Partiel — 2026-07-04 (script HMAC + mode persistant API/Sheets + smokes n8n dry-run OK ; smoke Unipile réel à exécuter) | v4-022→027 | 4h |
