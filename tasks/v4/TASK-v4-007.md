...
### Must Have
- [ ] Détecter `stop_reason === 'tool_use'` dans la réponse Anthropic SDK
- [ ] Si tool call = `get_calendly_slots` → appeler `calendly.getAvailableSlots(eventTypeUri)`
- [ ] Re-envoyer résultat des slots à Claude comme `tool_result` message
- [ ] Claude génère réponse finale avec slots au format naturel (via `formatSlotsNatural`)
- [ ] Fallback si 0 slots disponibles : message "Je peux vous proposer un créneau sur Calendly : [lien direct]"
- [ ] Tests Vitest : mock Anthropic retourne tool_use → vérifier que slots sont fetchés + réponse finale générée
...
## Acceptance Criteria
- [ ] `npm run test` — test tool call flow passe (mock Anthropic + mock calendly)
- [ ] Réponse finale contient les slots format naturel quand meeting_ready
- [ ] Fallback lien direct Calendly si 0 slots
- [ ] Pas d'appel Calendly si intent ≠ meeting_ready
