...
const SAMPLE: QualifiedLead = {
  ...
  prenom_gerant: "Jean",
  nom_gerant: "Martin",
  email_gerant: "jean.martin@acme.fr",
...
describe("campaign-export", () => {
  ...
  it("qualifiedLeadsToCsv — colonnes contact gérant + BOM UTF-8", () => {
    const csv = qualifiedLeadsToCsv([SAMPLE]);
    expect(csv.startsWith("\uFEFF")).toBe(true);
    const lines = csv.replace(/^\uFEFF/, "").split(/\r?\n/);
    expect(lines[0]).toContain('"score_reason"');
    expect(lines[0]).toContain('"email_gerant"');
  ...
  it("qualifiedLeadsToInstantlyCsv — en-têtes Instantly + filtre sans email", () => {
    const noEmail: QualifiedLead = { ...SAMPLE, email: "  ", email_gerant: "  ", lead_id: "l2" };
    const csv = qualifiedLeadsToInstantlyCsv([SAMPLE, noEmail]);
    const lines = csv.replace(/^\uFEFF/, "").split(/\r?\n/);
    expect(lines[0]).toContain("Email");
    expect(lines[0]).toContain("Personalization");
    expect(lines.length).toBe(2);
    expect(lines[1]).toContain("jean.martin@acme.fr");
    expect(lines[1]).toContain("Martin");
  });
