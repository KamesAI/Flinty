...
/** Pendant la génération, recharger les données serveur sans F5 manuel. */
const REFRESH_MS_WHEN_GENERATING = 20_000;

export function CampaignList({ campaigns }: CampaignListProps) {
  const router = useRouter();
  const hasGeneratingCampaign = campaigns.some((c) => c.isGenerating);

  useEffect(() => {
    if (!hasGeneratingCampaign) return undefined;
    const id = window.setInterval(() => {
      router.refresh();
    }, REFRESH_MS_WHEN_GENERATING);
    return () => window.clearInterval(id);
  }, [hasGeneratingCampaign, router]);
