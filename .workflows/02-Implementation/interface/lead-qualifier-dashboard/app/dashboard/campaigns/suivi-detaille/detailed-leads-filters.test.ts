...
describe("filterQualifiedLeads", () => {
  ...
  it("filtre par recherche texte sur société / gérant / email gérant / ville / hook / site", () => {
    const leads = [
      lead({ lead_id: "1", societe: "Alpha", email_gerant: "a@alpha.fr" }),
      lead({ lead_id: "2", prenom_gerant: "Béatrice", nom_gerant: "Martin" }),
      lead({ lead_id: "3", site: "zeta.com" }),
      lead({ lead_id: "4", ville: "Bordeaux", personalized_hook: "Travaux rénovation" }),
    ];
    ...
    expect(filterQualifiedLeads(leads, { ...emptyFilters, query: "rénovation" }).map((l) => l.lead_id)).toEqual(["4"]);
  });
...
describe("sortQualifiedLeads", () => {
  ...
  it("trie par champ texte alphabétiquement (société)", () => {
    const leads = [
      lead({ lead_id: "a", societe: "Charlie" }),
      lead({ lead_id: "b", societe: "alpha" }),
      lead({ lead_id: "c", societe: "Bravo" }),
    ];
    expect(
      sortQualifiedLeads(leads, { by: "societe", dir: "asc" }).map((l) => l.lead_id)
    ).toEqual(["b", "c", "a"]);
  });
