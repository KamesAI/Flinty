...
function DraggableCard({
  ...
  const siteDisplay = lead.site?.replace(/^https?:\/\//, "") ?? "";
  ...
  return (
    ...
    >
      {/* Row 1: drag handle + sector tag + score */}
      <div className="flex items-center gap-2 px-3 pt-2.5 pb-1">
        ...
        {lead.secteur && (
          <span className="truncate rounded bg-slate-100 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-widest text-slate-500 max-w-[130px]">
            {lead.secteur}
          </span>
        )}
      ...
      {/* Clickable body → fiche lead */}
      <Link
        href={`/dashboard/campaigns/${campaignId}/leads/${lead.lead_id}`}
        className="block px-3 pb-2.5 pt-0.5"
      >
        <p className="mb-1 truncate text-sm font-semibold leading-snug text-slate-900">
          {lead.nom}
        </p>
        ...
        {(lead.site || lead.téléphone) && (
          <div className="mb-2 flex min-w-0 items-center gap-3 text-[11px] text-slate-400">
            {lead.site && (
...
function CardOverlay({ lead }: { lead: KanbanLead }) {
  ...
  return (
    <div className="w-[300px] cursor-grabbing rounded-xl border border-primary/40 bg-white px-3 py-2.5 shadow-2xl">
      ...
      <span className="block truncate text-sm font-semibold text-slate-900">{lead.nom}</span>
    </div>
  );
}
